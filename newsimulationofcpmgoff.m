%gexinnmin 2019/07/3 houston 程序正确无误！
%%%共振条件下几种核磁共振谱的影响因素研究
%%构造谱研究扳倒角的影响；
%为了简单描述，令T1较大，可以认为是600ms
%%%new simulation code for the CPMG off resonance
%%%%%%构造T2谱
clear;
clc;
...
tau=TE/2;
%%%%确定好脉宽与TE的比例
%ratio=50 在第一次模拟中!在模拟回波间隔影响时，用20
ratio=20;
tp=TE/ratio;%ms
w1=flip1/tp;%单位KHZ*2pi
dw=0.02*w1;%单位KHZ*2pi
%NE number of echos
M1=zeros(3,NE,N);
%%开始循环
for i=1:N
    M1(:,:,i)=newoffcpmg(NE,T2(i),T1(i),tau,dw,w1,flip1,flip2,phi1,phi2,ispulse);
end
%开始做加权~~~~
[a,b,c]=size(M1);
M2=zeros(a,b);
